...
def _split_data(x_data, y_data=None, train_ratio=0.5, split_type='uniform'):
...
def load_windows_logs(log_file, label_file=None, window='session', train_ratio=0.5, split_type='sequential', save_csv=False, window_size=0):
    """Load Windows structured log into train and test data.
    ...
    ---------
        ...
        split_type: str, 'uniform' or 'sequential', determines how to split dataset.
        save_csv: bool, if True, saves the processed data to CSV.
        window_size: int, if > 0, slices data into smaller windows of specified size.
    ...
    if log_file.endswith('.npz'):
        ...
        assert window == 'session', "Only window=session is supported for Windows logs."
        ...
        for _, row in struct_log.iterrows():
            ...
            if event_id not in data_dict:
                data_dict[event_id] = []
            data_dict[event_id].append(row['Content'])
        ...
        if label_file:
            ...
            label_data = label_data.set_index('EventId')
            label_dict = label_data['Label'].to_dict()
            data_df['Label'] = data_df['EventId'].apply(lambda x: 1 if label_dict.get(x, 'Normal') == 'Anomaly' else 0)
            ...
            print(f"Training anomalies: {y_train.sum()}, Testing anomalies: {y_test.sum()}")
        ...
        if window_size > 0:
            ...
            log_summary = "{} {} windows ({}/{} anomaly), {}/{} normal"
            print(log_summary.format("Train:", x_train.shape[0], y_train.sum(), y_train.shape[0], (1 - y_train).sum(), y_train.shape[0]))
            print(log_summary.format("Test:", x_test.shape[0], y_test.sum(), y_test.shape[0], (1 - y_test).sum(), y_test.shape[0]))
            return (x_train, window_y_train, y_train), (x_test, window_y_test, y_test)

        if label_file is None:
            if split_type == 'uniform':
                split_type = 'sequential'
                print("Warning: Only split_type='sequential' is supported if label_file=None.")
            x_data = data_df['EventSequence'].values
            (x_train, _), (x_test, _) = _split_data(x_data, train_ratio=train_ratio, split_type=split_type)
            print(f"Total: {x_data.shape[0]} instances, train: {x_train.shape[0]} instances, test: {x_test.shape[0]} instances")
            print("x_train", len(x_train))

            return (x_train, None), (x_test, None), data_df

    else:
        raise NotImplementedError('load_windows_logs() only supports csv and npz files!')
    ...
    print(f"Total: {num_total} instances, {num_pos} anomaly, {num_total - num_pos} normal")
    print(f"Train: {num_train} instances, {num_train_pos} anomaly, {num_train - num_train_pos} normal")
    print(f"Test: {num_test} instances, {num_test_pos} anomaly, {num_test - num_test_pos} normal\n")
